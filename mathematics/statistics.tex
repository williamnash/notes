...
\begin{align}
P(k; \lambda) &= \lim_{n\to \infty} \frac{n(n-1)(n-2)...(n-k+1)}{k!}\left(\frac{\lambda}{n}\right)^k\left(1-\frac{\lambda}{n}\right)^{n-k}\\
...
\end{align}
this gives us an easily computable way to find edges of cumulative distributions.
...
\begin{align}
	p(\rho) \propto \binom{N}{A}\rho^A(1-\rho)^{N-A}
\end{align}
...
\begin{align}
	p(\rho) =\ffrac{ \binom{N}{A}\rho^A(1-\rho)^{N-A}}{\int_0^1\binom{N}{A}\rho^A(1-\rho)^{N-A} d\rho}
\end{align}
Since $\binom{N}{A}$ is constant and common in both numerator and denominator, we have
\begin{align}
...
\begin{align}
	p(\rho) =(N+1)\binom{N}{A}\rho^A(1-\rho)^{N-A}
\end{align}
...
\begin{align}
	...
	&= (N+1)\binom{N}{A}\int_0^1 ~\rho^{A+1}(1-\rho)^{N-A} d\rho
\end{align}
