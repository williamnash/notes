...
\begin{enumerate}
	\item $g_1g_2~\in ~G$ for all $g_1, g_2~\in~ G$. Which says when we multiply two elements, we get another element that is still within the group back
	\item $(g_1g_2)g_3 = g_1(g_2g_3)$ for $g_1, g_2, g_3~\in~ G$. Which says when given a specific order to multiply the elements, the sequencing of multiplication doesn't matter
	\item There is an identity element $e$ which gives $eg = ge = g$ for all $g\in G$. This says there is one element that does not change any of the other elements.
...
d_i = (2j_i + 1)
\end{align}
where $i ~\in~(a,b)$. So $d_i = 2$ in the case of spin 1/2 particles ($\uparrow$ and $\downarrow$). The dimension of their combination is the product of both dimensions
\begin{align}
d_{ab} = (2j_a+1)(2j_b+1)
\end{align}
